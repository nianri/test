...
<div style="margin-top: -464px;">
	<div style="margin-left: 627px;"><h3>油虎总后台统计</h3></div>
	<form id="exportfeeform" name="Form1"  method="post">
	<table border="1" style=" border-color: #c7d0d5; width: 100%">
		<tr><td colspan="8" style="height: 38px;border-bottom-color:#c7d0d5;background-color: #9f9fa81a;"><i class="Hui-iconfont">&#xe665;</i>  订单查询 <a href="javascript:void(0);" onclick="exportOrder()">申请导出</a><p style="margin-left: 231px;margin-top: 5px;">交易总金额：</p><p id="disAmount" style="margin-left: 333px;margin-top: -28px;"></p><p style="margin-left: 740px;margin-top: -28px;">总笔数：</p><p id="bishu" style="margin-left: 819px;margin-top: -31px;"></p>
			<p style="margin-left: 482px;margin-top: -30px;">平台总收：</p><p id="amount" style="margin-left: 567px;margin-top: -31px;"></p>
			</td></tr>
		<tr >
			<td colspan="4" width="13%"  align="center" style="height: 41px;border-right-color:#c7d0d5;border-bottom-color:#c7d0d5; " >创建时间:</td>
			<td   width="29%" style="border-right-color:#c7d0d5;border-bottom-color:#c7d0d5;"><input  placeholder="开始时间" class="form-control layer-date"  id="start"/><!-- <input type="text" onfocus="WdatePicker({ maxDate:'#F{$dp.$D(\'logmax\')||\'%y-%M-%d\'}' })" id="logmin" class="input-text Wdate" style="width:  109px;"> -->
				-
				<input  placeholder="结束时间" class="form-control layer-date"  id="end"/>
				<!-- <input type="text" onfocus="WdatePicker({ minDate:'#F{$dp.$D(\'logmin\')}',maxDate:'%y-%M-%d' })" id="logmax" class="input-text Wdate" style="width:  109px;"> -->
				</td>
				...
				<!--  <table border="1" style=" border-color: #c7d0d5; width: 50%">
			 	 ...
			 	 <input type="button" class="bgyuan addColor" value="当日"  onclick="button(this)">
				<input type="button" class="bgyuan" value="昨日" onclick="button(this)">
				<input type="button" class="bgyuan" value="本周" onclick="button(this)">
				<input type="button" class="bgyuan" value="上周"  onclick="button(this)">
				<input type="button" class="bgyuan"  value="本月" onclick="button(this)">
				<input type="button" class="bgyuan"  value="上月"onclick="button(this)">   
			</td>
		</tr>
		<tr>
			<td style=" height: 41px;border-right-color:#c7d0d5;border-bottom-color:#c7d0d5;" align="center" colspan="4">商户名称：</td>
			<td style="border-right-color:#c7d0d5;border-bottom-color:#c7d0d5;"><input type="text" style="height: 29px;width: 230px;border: 0.9" value="" id="storeName" ></td>
		...
		<tr style="height: 41px;">
			...
			<td style="border-right-color:#c7d0d5;border-bottom-color:#c7d0d5;"><input type="text" style="height: 29px;width: 230px;" id="storeNumber"></td>
	...
	<thead>
					<tr class="text-c">
						...
						<th width="60" style="background-color: #8a8a941a;">支付类型</th>
						<th width="60" style="background-color: #8a8a941a;">优惠券</th>
						<th width="60" style="background-color: #8a8a941a;">交易笔数</th>
						<th width="60" style="background-color: #8a8a941a;">交易总金额</th>
						<th width="60" style="background-color: #8a8a941a;">平台扣费</th>
						<th width="60" style="background-color: #8a8a941a;">油站实收</th>
...
<script>
setTimeout(function(){
	// location.href="GasStation.html?goods="+obj.value;
		 ...
		 _dataTables  = $("#sd").DataTable({
			    		...
			    		processing:false,
			              ...
			              ajax:{
			            	  ...
			            	  data:function(d){
			                   ...
			                   param.storeNumber=$("#storeNumber").val();
			                  // console.log(d.sEcho+"&&&&&&&");
			    	...
			    	//},
			         ...
			         columns: [
						{ "data": "shopName"},
						{ "data": "storeNumber"}, 
			             { "data": "goodsName","render":function(data,type,full,callback){
								var  selectCounts =$("#selectCounts").val();
								if(selectCounts!=''){
									return data;
								}else{
									return '';
			           ...
			           {"sClass":"text-c",
			            	 "render":function(data, type,row){
			            		 var title="产品查看";
			          		   var url="../order/zLookOrder?id="+row.id;
...
function selectCounts(obj){
 ...
 function counts(){
	 $.post("../order/zCountOrders",{"logmin":$("#start").val(),"logmax":$("#end").val(),"select":$("#select").val(),"selectCounts":$("#selectCounts").val(),"storeName":$("#storeName").val(),"storeNumber":$("#storeNumber").val()},function(data){
		 if(data.disAmount!=null){
			 $("#disAmount").html(data.disAmount+"(元)");     
		 }else{
			 $("#disAmount").html(0+"(元)");
		 }
		 if(data.bishu!=null){
			 $("#bishu").html(data.bishu+"(笔)");
		 }else{
			 $("#bishu").html(0+"(笔)");
		 }
...
function button(obj){
	...
	if(obj.value!=''&&obj.value=='本周'){
		...
		$("#start").val(dat);
		$("#end").val(date.format('yyyy-MM-dd hh:mm:ss.S')); 
	}
...
/* 	$.post("../order/exportOrder",{"logmin":$("#logmin").val(),"logmax":$("#logmax").val(),"select":$("#select").val(),"selectCounts":$("#selectCounts").val()},function(){
		
	}); */
	document.Form1.action="../order/exportOrder?logmin="+$("#start").val()+"&logmax="+$("#end").val()+"&select="+$("#select").val()+"&selectCounts="+$("#selectCounts").val();
