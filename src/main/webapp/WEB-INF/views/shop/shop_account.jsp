...
<script type="text/javascript">
			...
			{field : 'action',title : '操作',width : 150,align : "center",
				formatter : function(cellvalue,options, row) {
					...
					html += "<a href='javascript:void(0);' class='btn btn-primary' onclick='editshop(\""
							+ options.shopid + "\",\""+ options.status+ "\")' >编辑</a>  ";
	...
	function editshop(shopid,status) {
		if(status=="3"||status=="4"){
			toastr.warning("该加油站不可编辑！");
		}else{
			var index = layer.open({type : 2,title : "加油站-信息编辑",
				content : "${pageContext.request.contextPath}/shop/shopEditInfo/"  + shopid + ""	});
			layer.full(index);
		}
	}
</script>
